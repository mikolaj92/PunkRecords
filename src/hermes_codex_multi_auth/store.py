...
class AccountRepository:
    ...
    def resolve_account(self, ident: str) -> AccountRecord:
        ...
        for account in state.accounts:
            if ident in {account.id, account.account_id, account.label}:
                return account
    ...
    def upsert_account(self, account: AccountRecord, *, make_active: bool = True) -> AccountRecord:
        ...
        for index, existing in enumerate(state.accounts):
            if existing.account_id == account.account_id or existing.id == account.id:
    ...
    def list_proxy_candidates(self) -> list[AccountRecord]:
        state = self.load()
    ...
    def mark_proxy_failure(self, account_id: str, *, error: str, cooldown_seconds: int = 60) -> None:
        state = self.load()
    ...
    def mark_proxy_success(self, account_id: str) -> None:
        state = self.load()
        for account in state.accounts:
            if account.account_id == account_id:
