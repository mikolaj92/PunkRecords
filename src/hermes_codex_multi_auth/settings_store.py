...
def _validate_settings_patch(payload: dict[str, Any], *, path: str = "settings") -> None:
    ...
    for key, value in payload.items():
        ...
        if key == "proxy":
            ...
            if not isinstance(value, dict):
                raise ValueError("settings.dashboard must be an object")
            allowed_dashboard = {"enabled"}
            for dashboard_key, dashboard_value in value.items():
                if dashboard_key not in allowed_dashboard:
                    raise ValueError(f"Unsupported settings key: dashboard.{dashboard_key}")
                if dashboard_key == "enabled" and not isinstance(dashboard_value, bool):
                    raise ValueError("settings.dashboard.enabled must be a boolean")
